...
async function runImport() {
    ...
    if (!supabaseUrl || !serviceRoleKey) {
        console.error('Faltam variáveis necessárias.');
        return;
    }
    ...
    try {
        ...
        // Atribuir todas as gestantes que não tenham usuário atrelado à robsonlucasfotos@gmail.com
        let userId = '73bdf7da-9a61-4442-af05-aade8aaac8aa'; // Robson's ID

        // Verifica se a tabela existe de fato, forçando refresh se der erro
        const ping = await supabase.from('pregnant_women').select('id').limit(1);
        if (ping.error) {
            console.error("ALERTA: O Supabase ainda não conseguiu ler a tabela, ou ela não foi criada direito:");
            console.error(ping.error);
            return;
        }

        console.log(`Using user ID (Robson): ${userId}`);

        for (let p of patients) {

            p.risk_level = p.risk_level.toLowerCase() === 'alto' ? 'Alto' : 'Habitual';

            const { error: insertErr } = await supabase.from('pregnant_women').insert([{
                user_id: userId,
                name: p.name,
                dum: new Date().toISOString(), // DUM placeholder
            ...
            if (insertErr) {
                console.error(`Erro inserindo ${p.name}:`, insertErr);
            } else {
