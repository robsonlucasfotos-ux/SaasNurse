...
data_gestantes = """BRENDA KAROLAYNY MARQUES ALCIDES	02/07/2007	(61) 9147-0984	10/09/2025	35	Alto	SIFILI E MIOMA
LUCIANA  DE PAIVA VIEIRA 	15/11/1985	(61)992408378	22/08/2025	18	Habitual	
SUZANA KELLY SILVA SABINO	09/06/2016	(61)981063849	04/09/2025	15	Alto	SIFILI
MIKAELE DE JESUS VIEIRA	15/07/2002	(61)996552327	02/09/2025	23	Habitual	
LILIAN CARVALHO NONATO DA SILVA	04/06/1998	(61)996228365	09/09/2025		Alto	DESCOLAMENTO DE PLACENTA
KAREN VITORIA RODRIGUES DE SOUSA	02/01/2004	(61)99100-1686	22/08/2025	21	Alto	SIFILIS
JULLYANA NUNES DA SILVA	21/03/2023	(61)996343410		22	Habitual	
ALINE SILVA OLIVEIRA	11/12/2001	(61)99376-1099	04/07/2025	23	Habitual	
DEBORA ANARDA PEREIRA DE ARAIUJO	13/04/1987	(61)992217754	22/08/2025		Habitual	
SARAN DAYSE SILVA  DOS SANTOS	02/07/1997	(61)998374197	07/08/2025	28	Habitual	
BRENDA RODRIGUES DE ARAUJO	03/11/2000	(61)99151-3684	13/08/2025	24	Habitual	
RAFAELA SANTOS FARIAS	06/02/2025	61 98120-6236	02/10/2025	20	Habitual	
BIANCA SOUZA DA SILVA	30/08/2004	61 98505 7982	03/10/2025	21	Habitual	
ISABELA PEREIRA DA SILVA	10/02/2005	61099449-1695	29/10/2025	20	Habitual	
PATRICIA OLIVEIRA LIMA	03/09/1986	(61) 98234-9334	13/11/2024	39	Habitual	
VITORIA CONCEIÇÃO SOUZA	26/05/2001	61 99465-1011	01/07/2025	24	Habitual	
Sophie Rafaelly Ribeiro Amorim	24/06/2011	(61) 99187-2626	29/12/2025	14	Habitual	
MARIA EDUARDA MELO DA SILVA SOUSA	26/12/2002	(61) 99104-4458	05/01/2026	23	Intermediário	ABORTO
SABRINA ESTER DE LIMA BRITO	10/04/20009	(61) 992183732	09/12/2025	-17995	Habitual	
LEANDRA BENILDA LIMA DE ALBURQUEQUE	13/07/1998	(61)999218929	30/12/2025	27	Habitual	
Jaine Freire Pereira	30/07/1997	(61) 99545-9431	07/09/2025	28	Alto	SIFILIS
Nicolly Vieira Amaral	28/04/2006	61 992794083	13/01/2026	19	Habitual	
Amanda Costa Melo Alves	01/06/1996	(61) 99437-5550	15/01/2026	29	Alto	ABORTO"""
...
for line in data_gestantes.splitlines():
    ...
    if len(cols) >= 6:
        ...
        out_sql.append(f"  INSERT INTO pregnant_women (user_id, name, age, birth_date, phone, risk_level, risk_reason) VALUES (v_user_id, '{name}', {age}, {bdate}, {phone_sql}, '{risk}', {reason_sql});")
...
for line in data_criancas.splitlines():
    ...
    if len(cols) >= 2:
        name = cols[0].strip().replace("'", "''")
        bdate = format_date(cols[1])
        # Sometimes there's risk in col 4
        risk = cols[4].strip() if len(cols) > 4 and cols[4].strip() else 'Habitual'
        # Normalize risk to valid values: Habitual, Intermediário, Alto
...
for line in data_cronicos.splitlines():
    ...
    if len(cols) >= 2:
        ...
        # Map conditions to valid front-end choices: HAS, DM, Ambos -> HAS e DM
        ...
        # Map risk to valid choices: Baixo, Moderado, Alto -> user had "Médio", "Muito Alto"
...
with open("C:\\\\Users\\\\robso\\\\.gemini\\\\antigravity\\\\brain\\\\5d9672a1-8262-4fde-8b7a-080357660b69\\\\seed_patients.sql", "w", encoding="utf-8") as f:
    f.write("\\n".join(out_sql))
